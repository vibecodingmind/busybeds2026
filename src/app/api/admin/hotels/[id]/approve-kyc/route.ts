...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    await db.hotel.update({ where: { id: params.id }, data: { partnershipStatus: 'ACTIVE', status: 'active' } });
    const owner = await db.hotelOwner.findUnique({ where: { hotelId: params.id } });
