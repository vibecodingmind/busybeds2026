...
async function seed() {
  ...
  for (const pkg of packages) {
    await db.subscriptionPackage.upsert({
      where: { id: `pkg-${pkg.tier}` },
      update: pkg,
      create: { id: `pkg-${pkg.tier}`, ...pkg },
    });
  }
  console.log('✅ Ensured 4 subscription packages');

  const adminHash = await hashPassword('Admin123!');
  const admin = await db.user.upsert({
    where: { email: 'admin@busybeds.com' },
    update: {},
    create: { email: 'admin@busybeds.com', passwordHash: adminHash, fullName: 'Admin User', role: 'admin', emailVerified: true, referralCode: 'REF-ADM01' },
  });
  console.log('✅ Ensured admin user');

  const ownerHash = await hashPassword('Owner123!');
  const owner = await db.user.upsert({
    where: { email: 'owner@busybeds.com' },
    update: {},
    create: { email: 'owner@busybeds.com', passwordHash: ownerHash, fullName: 'Hotel Owner', role: 'owner', emailVerified: true, referralCode: 'REF-OWN01' },
  });
  console.log('✅ Ensured owner user');

  // Check if travelers already exist
  const existingTravelers = await db.user.findMany({ where: { role: 'traveler' } });
  if (existingTravelers.length === 0) {
    ...
    for (let i = 0; i < names.length; i++) {
      await db.user.create({ data: { email: names[i].toLowerCase().replace(/\s+/g, '.') + '@example.com', passwordHash: hash, fullName: names[i], role: 'traveler', emailVerified: true, referralCode: `REF-T${String(i+1).padStart(2,'0')}` } });
    }
    console.log('✅ Created 20 travelers');
  } else {
    console.log(`✅ ${existingTravelers.length} travelers already exist, skipping`);
  }

  // Check if hotels already exist
  const existingHotels = await db.hotel.count();
  if (existingHotels === 0) {
    const hotelData = [
    ...
    const travelers = await db.user.findMany({ where: { role: 'traveler' } });
    const hotels = [];
    ...
    for (const h of hotelData) {
      ...
      const hotel = await db.hotel.create({ data: {
        ...
        isPartner: h.partnership === 'ACTIVE',
        partnerDiscountPercent: h.partnership === 'ACTIVE' ? h.discountPercent : null,
      } });
      hotels.push(hotel);
    ...
    await db.hotelOwner.upsert({
      where: { id: 'owner-hotel-link' },
      update: {},
      create: { id: 'owner-hotel-link', userId: owner.id, hotelId: hotels[0].id, kycStatus: 'approved' },
    });
    console.log(`✅ Created ${hotels.length} hotels with rooms`);

    // Subscriptions
    const pkgs = await db.subscriptionPackage.findMany();
    const subs = [];
    ...
    for (let i = 0; i < 80; i++) {
      ...
      await db.coupon.create({ data: {
        code, userId: travelers[i % travelers.length].id, hotelId: hotels[i % hotels.length].id, subscriptionId: subs[i % subs.length].id,
    ...
    // Flash deals
    const existingDeals = await db.flashDeal.count();
    if (existingDeals === 0) {
    ...
    await db.siteSettings.upsert({
      ...
      update: {},
      create: { key: 'currency_rates', value: '{"USD":1,"TZS":2750,"KES":155}' },
    });
  } else {
    console.log(`✅ ${existingHotels} hotels already exist, skipping full seed`);
  }
